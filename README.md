...
```text
/翻唱 [模型] [人声变调] [BGM变调] [人声音量] [背景音乐音量]
/翻唱下载 [模型] [人声变调] [BGM变调] [人声音量] [背景音乐音量]
```
...
音量为 `0` 表示静音。`/翻唱下载` 一定发送 MP3 文件，不受插件“将结果作为语音消息发送”开关影响。直接调用后端 API 时音量上限为 `3`，AstrBot 插件为便于日常使用将命令和配置范围限制为 `0`～`2`。

## 分离缓存

服务使用上传文件原始字节的 SHA-256 作为缓存键，并在缓存清单中记录上传文件的歌名。相同文件再次翻唱时会复用干人声和伴奏，与模型、人声变调、BGM 变调和文件名无关。升级到带歌名清单的缓存布局后，旧版无歌名缓存会在服务启动时自动清理。
...
`POST /cover` 使用 `multipart/form-data`，必填字段为 `audio` 和 `model`。可选字段为 `transpose`（人声变调）、`instrumental_transpose`（BGM 变调）、`index_rate`、`rms_mix_rate`、`protect`、`vocal_gain` 和 `instrumental_gain`。两个变调字段均为 -24～24 的整数，默认 `0`。启用令牌后，所有接口都需要请求头 `X-AI-Cover-Token`。
