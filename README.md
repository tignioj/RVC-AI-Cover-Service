...
- 支持分别设置人声和 BGM 变调，BGM 变调时保持伴奏时长；
...
/翻唱 珊瑚宫心海 0 2
/翻唱 珊瑚宫心海 0 2 1.3
/翻唱 珊瑚宫心海 0 2 1.3 0.8
/翻唱 珊瑚宫心海 0 2 1.3 0.8 1.25
/翻唱下载
/翻唱下载 珊瑚宫心海 0 2 1.3 0.8 1.25
/翻唱状态
```
...
```text
/翻唱 [模型] [人声变调] [BGM变调] [人声音量] [背景音乐音量] [变速]
/翻唱下载 [模型] [人声变调] [BGM变调] [人声音量] [背景音乐音量] [变速]
```

| 参数 | 范围 | 默认行为 |
| --- | --- | --- |
| 模型 | `/翻唱模型` 返回的名称 | 使用插件配置中的默认模型，初始为“芙宁娜” |
| 人声变调 | `-24`～`24` 的整数 | `0` |
| BGM 变调 | `-24`～`24` 的整数 | `0`，变调时保持伴奏时长 |
| 人声音量 | `0`～`2` | 使用插件配置，`1` 为原音量 |
| 背景音乐音量 | `0`～`2` | 使用插件配置，`1` 为原音量 |
| 变速 | `0.5`～`2` | `1`（原速） |

音量为 `0` 表示静音。变速大于 `1` 会加快、小于 `1` 会减慢，并保持音高不变。`/翻唱下载` 一定发送 MP3 文件，不受插件“将结果作为语音消息发送”开关影响。直接调用后端 API 时音量上限为 `3`，AstrBot 插件为便于日常使用将命令和配置范围限制为 `0`～`2`。

## 分离缓存

服务使用上传文件原始字节的 SHA-256 作为缓存键，并在缓存清单中记录上传文件的歌名。相同文件再次翻唱时会复用干人声和伴奏，与模型、人声变调、BGM 变调、变速和文件名无关。升级到带歌名清单的缓存布局后，旧版无歌名缓存会在服务启动时自动清理。

```text
GET    /cache          查看缓存统计和歌曲列表，可用 query 参数搜索
POST   /cache/delete   按缓存 ID 删除一个或多个条目
DELETE /cache          清空全部分离缓存
```

AstrBot 插件详情页也提供“分离缓存”管理页面，可搜索歌名、单独清理或批量清理。删除缓存会等待当前 GPU 任务结束，不会删除正在使用的音轨。
...
| `GET` | `/cache` | 查看或搜索分离缓存歌曲列表 |
| `POST` | `/cache/delete` | 按 `ids` 数组批量删除缓存 |
| `DELETE` | `/cache` | 清空分离缓存 |

`POST /cover` 使用 `multipart/form-data`，必填字段为 `audio` 和 `model`。可选字段为 `transpose`（人声变调）、`instrumental_transpose`（BGM 变调）、`index_rate`、`rms_mix_rate`、`protect`、`vocal_gain`、`instrumental_gain` 和 `speed`（变速）。两个变调字段均为 -24～24 的整数，默认 `0`；`speed` 范围为 0.5～2，默认 `1`。启用令牌后，所有接口都需要请求头 `X-AI-Cover-Token`。
